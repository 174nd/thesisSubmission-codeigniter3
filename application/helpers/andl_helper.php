...
{
  $ci = get_instance();
  if (!$ci->session->userdata('username') || !$ci->session->userdata('password') || !$ci->session->userdata('akses')) {
    $ci->session->set_flashdata('message', $ci->lang->line('alert-kicked'));
    $ci->session->set_flashdata('alert', ['danger', 'icon fas fa-exclamation-triangle', 'Error!', 'Tolong Log-In Terlebih Dahulu!']);
    redirect('auth/logout');
  } else {
    if ($akses != '' && $ci->session->userdata('akses') != $akses) {
      $ci->session->set_flashdata('alert', ['danger', 'icon fas fa-exclamation-triangle', 'Error!', 'Tolong Log-In Terlebih Dahulu!']);
      redirect('auth/logout');
    } else {
      if ($ci->db->get_where('user', [
...
function set_sidebar($akses)
{
  ...
  if ($akses == 'admin') {
    return [
      'Dashboard'     => ['fas fa-window-restore', base_url('admin')],
      'Mahasiswa'     => ['fas fa-user-graduate', base_url('adm_mahasiswa')],
      'Dosen'         => ['fas fa-user-tie', base_url('adm_dosen')],
      'Pimpinan'      => ['fas fa-users', base_url('pimpinan')],
      'Konsentrasi'   => ['fas fa-graduation-cap', base_url('konsentrasi')],
      'Program Studi' => ['fas fa-school', base_url('prodi')],
      'Fakultas'      => ['fas fa-landmark', base_url('fakultas')],
      'User'          => ['fas fa-users-cog', base_url('user')],
...
function set_date($tanggal, $format_file = 'Y-m-d', $format_hasil = '%Y-%m-%d')
{
  // setlocale(LC_ALL, 'id-ID', 'id_ID');
  // https://www.php.net/manual/en/datetime.format.php
  // https://www.php.net/manual/en/function.strftime.php
  $let = DateTime::createFromFormat($format_file, $tanggal)->getTimestamp();
  return strftime($format_hasil, $let);
}
